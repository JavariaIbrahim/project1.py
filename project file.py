...
def login():
    ...
    if username == "javaria" and password == "2596":
...
def add_book():
    ...
    if not is_book_present(book_id):
        with open(BOOKS_CSV_FILE, 'a', newline='\r\n') as f:
            writer = csv.writer(f)
...
def return_issued_book():
    ...
    with open(ISSUED_BOOKS_CSV_FILE, 'r', newline='\r\n') as f_issued:
...
def display_dashboard(admin_name):
    ...
    print("\n=== Dashboard ===")
    print(f"Logged-in Admin: {admin_name}")
...
def admin_panel():
    ...
    if choice == "1":
        if login():
            admin_name = "javaria"  # Replace with the actual admin name
            display_dashboard(admin_name)
            return True 
            # If login is successful, you can perform other admin tasks here.
...
while True:
    if admin_panel():
        # If login is successful, call the dashboard function
        dashboard()
    else:
        # If login is not successful, you might want to handle it here
        pass
